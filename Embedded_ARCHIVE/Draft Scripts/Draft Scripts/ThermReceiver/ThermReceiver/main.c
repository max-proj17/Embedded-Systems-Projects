...
#define CE_PIN PORTD5 // Use pin PD7 as the SS (CS) pin
#define CSN_PIN PORTD5
...
//PORTB0 is CSN
void spi_init(void) {
	DDRB |= (1<<DDB3) |(1<<DDB5); // Set MOSI, SCK as  outputs
	DDRB &= ~(1<<DDB4); //Set MISO as input
	DDRD |= (1<<DDD4) | (1<<DDD5); //set CE and CSN as output
